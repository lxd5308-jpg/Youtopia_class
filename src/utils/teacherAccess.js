...
// authenticated (settings/private is teacher-only).
//
// The read itself is the check: the Firestore rules only allow settings/private
// to be read by an account already on the allow-list (or a hard-coded root
// teacher), so a student's fetch is denied and falls through to `false`.
